...
$ guacone collect s3 --s3-url http://localhost:9000 --s3-bucket guac-test --poll --s3-mp-endpoint localhost:9092 --s3-queues sboms
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		emit := func(d *processor.Document) error {
			_, err := ingestor.Ingest(ctx, d, s3Opts.graphqlEndpoint, transport, csubClient, s3Opts.queryVulnOnIngestion, s3Opts.queryLicenseOnIngestion, s3Opts.addVulnMetadata)
			if err != nil {
				errFound = true
...
func validateS3Opts(graphqlEndpoint, headerFile, csubAddr, s3url, s3bucket, s3path, region, s3item, mp, mpEndpoint, queues string,
	csubTls, csubTlsSkipVerify, poll bool, queryVulnIngestion bool, queryLicenseIngestion bool, addVulnMetadata bool,
) (s3Options, error) {
	var opts s3Options
	...
	opts = s3Options{
		s3url, s3bucket, s3path, s3item, region, queues, mp, mpEndpoint, poll, graphqlEndpoint, headerFile,
		csubClientOptions, queryVulnIngestion, queryLicenseIngestion, addVulnMetadata,
	}
